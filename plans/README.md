...
| 009  | `matricula` única por gestora, não globalmente (achado #15) | P1 | M | 008 | DONE (branch `advisor/009-matricula-unica-por-gestora`, worktree `~/Projetos/prisma-worktrees/plan-009`, commits `aebcd39`+`9310e75`, 2026-07-21 — migration testada com upgrade/downgrade/upgrade real. Desenho final ficou mais conservador que o plano original: `gestora_id` em `LoginReq` é OPCIONAL (não obrigatório) — chamadores antigos/sem colisão continuam funcionando sem mudança, só recusa (genérico, sem 500) quando a matrícula existe em mais de uma gestora e nenhum `gestora_id` foi informado; evitou reescrever ~71 call sites de teste que assumiam matrícula sozinha. Tela de login ganhou seletor de gestora mesmo assim, pra sempre desambiguar na prática. Suíte completa 241 passed/6 skipped sem regressão, `tsc`/`vitest` do frontend limpos (achado de teste: Base UI Select monta a Collection vazia se aberto antes do fetch assíncrono de gestoras resolver — corrigido esperando o fetch antes de interagir). Mergeado em `feature/copiloto-analitico` (merge commit `8c3a65d`).) |
