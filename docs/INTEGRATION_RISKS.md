...
| 4 | Intranet sem npm/pip públicos | build em prod | Bundle Next.js pré-compilado + mirror interno (Artifactory) para Python |
| 5 | Licença Bloomberg | jurídico | Dados VaR nunca expostos por API pública; permanecem no perímetro VDI |
| 6 | Encoding legado (latin-1/UTF-8) | dados corrompidos | Normalização na ingestão (lição documentada do performance-attribution) |
